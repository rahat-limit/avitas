...
class _PushNotificationsHandlerState extends State<PushNotificationsHandler> {
  ...
  Widget build(BuildContext context) => _loading
      ? Container(
          color: Colors.black,
          child: Image.asset(
            'assets/images/photo_2024-06-29_14-43-51.jpg',
            fit: BoxFit.contain,
          ),
        )
      : widget.child;
...
final parametersBuilderMap =
  ...
  'auth_2_createProfile': (data) async => ParameterData(
        allParams: {
  ...
  'DetailPage': (data) async => ParameterData(
        allParams: {
          'job': await getDocumentParameter<JobRecord>(
              data, 'job', JobRecord.fromSnapshot),
          'worker': await getDocumentParameter<WorkerRecord>(
              data, 'worker', WorkerRecord.fromSnapshot),
        },
      ),
  ...
  'UserPage': (data) async => ParameterData(
        allParams: {
  ...
  'SearchUsers': (data) async => ParameterData(
        allParams: {
          'job': getParameter<DocumentReference>(data, 'job'),
        },
      ),
  ...
  'PaymentRecieved': (data) async => ParameterData(
        allParams: {
          'job': await getDocumentParameter<JobRecord>(
              data, 'job', JobRecord.fromSnapshot),
          'paymentDate': getParameter<DateTime>(data, 'paymentDate'),
        },
      ),
};
